...
document.querySelector('#ordenar').addEventListener('change', (event) => {
    ...
    // Função para extrair o preço do elemento
    const extrairPreco = (elemento) => {
        const textoPreco = elemento.querySelector('.precoProduto').textContent;
        const preco = parseFloat(textoPreco.match(/\d+(\.\d+)?/)[0]);
        return preco;
    };

    if (selecionado === "decrescente") {
        produtosArray.sort((a, b) => extrairPreco(b) - extrairPreco(a));
    } else if (selecionado === "crescente") {
        produtosArray.sort((a, b) => extrairPreco(a) - extrairPreco(b));
    }
